...
namespace RoomReservation
{
    public partial class MainUI : Form
    {
        ...
        private List<BookingDTO> bookingListAll = null;
        ...
        public MainUI()
        {
            ...
            bookingListAll = new List<BookingDTO>();
            roomBLL = new RoomBLL();
            roomDTO = new RoomDTO();
        ...
        private void btnBookingSave_Click(object sender, EventArgs e)
        {
            if (cbxRoom.SelectedItem != null)
            {
                    ...
                    (cbxEndHour.SelectedItem != null && cbxEndHour.SelectedItem != ""))
                {
                    ...
                    {
                        LoadData(i);
                        bookingBLL.ManipulateBooking(bookingDTO);
                        bookingBLL.SaveBooking();
                        bookingListAll = Functions.ConvertToList<BookingDTO>(bookingBLL.LoadXML());
                    }
        ...
        {
            if (dgvQuickBookings.SelectedCells[0].Tag != null)
            {
                ...
                if (date >= DateTime.Today)
                {
                    if (cbxRoom.SelectedItem != null)
                        if (!string.IsNullOrEmpty(dgvQuickBookings.SelectedCells[0].Value.ToString()) && !dgvQuickBookings.CurrentRow.Cells["Horário"].Selected)
                            ...
                            {
                                bookingBLL.DeleteBooking(dgvQuickBookings.SelectedCells[0].Tag.ToString());
                                LoadBookings();
                                LoadAllBookings();
                            }
                }
        ...
        private void dgvAllBookings_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 6)
            {
                if (MessageBox.Show("Deseja excluir o agendamento?", "Mecalux", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                {
                    bookingBLL.DeleteBooking(dgvAllBookings.SelectedCells[0].Value.ToString());
                    LoadBookings();
                    LoadAllBookings();
                }
            }
        ...
        private void LoadRooms()
        {
            ...
            cbxRoom.DataSource = roomBLL.LoadXML();
        }
